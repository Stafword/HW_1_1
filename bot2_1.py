...
class AddressBook(UserDict):
    ...
    def printBook(self):
        ...
        for rec in self.data:
            if len(str(self.data[rec].getBirthday())) == 19:
                print(
                    f"{id:3}. {self.data[rec].getName():12} тел. {self.data[rec].getPhones()}\n{' ':17} д.н. {self.data[rec].getBirthday().strftime('%d.%m.%Y')}"
                )
                id += 1
            else:
                print(
    ...
    def delete(self, name):
        if self.ifExist(name):
            ...
            print(
                f"{Fore.RED}Запис {Fore.GREEN}{name} {Fore.RED}не знайдено.{Fore.RESET}"
            )
            return self
    ...
    for user in users:
        if len(str(users[user].getBirthday())) == 19:
            tempt_b_day = datetime(
                year=today.year,
                month=user.getBirthday().month,
                day=user.getBirthday().day,
            )
